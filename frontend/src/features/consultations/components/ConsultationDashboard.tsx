...
function canJoinConsultation(slotTime: string) {
  ...
  // Use today's date for doctor's dashboard since all are "today"
      ...
      {startingSoonAppts.length > 0 && !activeAppt && (
        <div style={{ background: 'var(--primary)', color: 'white', padding: '12px 16px', borderRadius: 'var(--radius)', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: '0.85rem', marginTop: 4 }}>Your consultation with {startingSoonAppts[0].patient_name} is starting.</div>
          </div>
              ...
              upcoming.map(a => (
                <div key={a.id} style={{ borderBottom: '1px solid var(--border)', padding: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{ color: 'var(--primary)', fontWeight: 600 }}>{formatTime12H(a.slot_time)}</span>
                  </div>
                  ...
                  {!activeAppt && (
                    <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                      {canJoinConsultation(a.slot_time) && a.consultation_type === 'online' ? (
                        <button className="btn btn-primary btn-sm" style={{ flex: 1 }} onClick={() => handleStart(a, true)}>Start & Join</button>
                      ) : (
                        <button className="btn btn-primary btn-sm" style={{ flex: 1 }} onClick={() => handleStart(a, false)}>Start</button>
                      )}
                      <button className="btn btn-secondary btn-sm" style={{ flex: 1 }} onClick={() => setActiveAppt(a)}>Prepare</button>
                    </div>
                  )}
                </div>
              ))
